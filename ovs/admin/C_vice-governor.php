...
$position = 'Vice-Governor'; // Change for each position file
...
<div class="modern-tab-bar">
	...
	$query = mysqli_query($conn, "SELECT * FROM candidate WHERE Position='Vice-Governor' LIMIT 1");
	if(mysqli_num_rows($query) > 0) {
	    $row = mysqli_fetch_array($query);
	    $id_excel = $row['CandidateID'];
	} else {
	    $id_excel = ''; // Set default value if no candidates exist
	}
	?>
...
<?php while($candidate_rows=mysqli_fetch_array($candidate_query)){ $id=$candidate_rows['CandidateID'];
...
<tr class="del<?php echo $id ?>">
	...
	<td align="center"><img class="pic" width="40" height="30" src="<?php echo $candidate_rows['Photo'];?>" border="0" onmouseover="showtrail('<?php echo $candidate_rows['Photo'];?>','<?php echo $candidate_rows['FirstName']." ".$candidate_rows['LastName'];?> ',200,5)" onmouseout="hidetrail()"></a></td>
		<td align="center">
	<?php echo $candidate_rows['vote_count']; ?>
</td>	
