...
$qryreport = mysqli_query($conn, 
    ...
    AND v.academic_year = '$current_academic_year'
...
$sheet->setCellValue('A3', 'Unvoted Voters List');
...
while ($reportdisp = mysqli_fetch_array($qryreport)) {
    ...
    $sheet->setCellValue('A' . $row, $reportdisp['StudentID'])
          ...
          ->setCellValue('F' . $row, 'Not Voted');
    $row++;
}

// Add total count row
$totalRow = $row;
$sheet->setCellValue('A' . $totalRow, 'TOTAL UNVOTED VOTERS');
...
header('Content-Disposition: attachment;filename="Unvoted_Voters_List_' . date('Y-m-d') . '.xlsx"');
