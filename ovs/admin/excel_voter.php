...
$qryreport = mysqli_query($conn,
    "SELECT * FROM voters 
     WHERE TRIM(academic_year) = TRIM('$academic_year')
     AND TRIM(UPPER(Enrollment)) = 'ENROLLED'
...
$sheet->setCellValue('A3', 'Enrolled Voters List');
...
while ($reportdisp = mysqli_fetch_array($qryreport)) {
    ...
    $sheet->setCellValue('A' . $row, $reportdisp['StudentID'])
          ...
          ->setCellValue('F' . $row, $reportdisp['Status']);
    $row++;
}

// Add total count row
$totalRow = $row;
$sheet->setCellValue('A' . $totalRow, 'TOTAL ENROLLED VOTERS');
...
header('Content-Disposition: attachment;filename="Enrolled_Voters_List_' . date('Y-m-d') . '.xlsx"');
